...
def cosinesimilarity(self, input_string):
    ...
    vectorizerX = (
        TfidfVectorizer()
    )  # THIS WILL AUTOMATICALLY GENERATE OUR TF-IDF VECTORIZER
    vectorizerX.fit(clean_corpus)  # THIS WILL FIT OUR CORPUS INTO THE VECTORIZER
    doc_vector = vectorizerX.transform(
        clean_corpus
    )  # TRANSFORMING THE CORPUS INTO A VECTOR
    ...
    if not CosineSimilarity.objects.all():
        ...
        for i in output:
            ...
            CosineSimilarity.objects.create(
                title=i["title"], link=i["link"], score=i["score"], angle=i["angle"]
            )
